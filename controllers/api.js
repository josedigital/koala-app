...
router.get('/api/jobs/:searchTerm', function(req, res, next) {
    console.log("inside keyword search helper function" + req.params.searchTerm);
    jobsArr = [];
    ...
    indeedUri = "http://api.indeed.com/ads/apisearch?publisher=180200156283414&q=" + searchTerm +
        ...
        "&userip=1.2.3.4&useragent=Mozilla/%2F4.0%28Firefox%29&v=2&format=json";
    // dice search   
    request({ uri: 'http://service.dice.com/api/rest/jobsearch/v1/simple.json?text=' + searchTerm },
        function(error, response) {
            console.log("Dice Search Reponse");
            if (!error && response.statusCode === 200) {
                var resultsDice = JSON.parse(response.body);
